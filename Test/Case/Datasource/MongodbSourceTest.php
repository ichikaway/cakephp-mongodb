 ...
 */
	public function testCreateConnectionName() {
		...
		$host = $this->mongodb->createConnectionString($config);
...
/**
 * Tests the Model::getMongoDb() call Mongodb::getMongoDb
 *
 * @return void
 * @access public
 */
 ...
 */
	public function testGetMongoCollection() {
		...
		$this->assertTrue(is_object($obj));
		$objName = get_class($obj);
		$this->assertEqual('MongoDB\Collection', $objName);
	}
 ...
 */
	public function testSave() {
		...
		$this->assertNotEmpty($saveResult);
		...
		$this->assertTrue(is_a($resultData['created'], 'MongoDB\BSON\UTCDateTime'));
		$this->assertTrue(is_a($resultData['modified'], 'MongoDB\BSON\UTCDateTime'));
	}
 ...
 */
	public function testSetMongoUpdateOperator() {
		...
		$this->Post->mongoNoSetOperator = null;
	}
 ...
 */
	public function testUpdateWithoutMongoSchemaProperty() {
		...
		syslog(LOG_NOTICE, "MongoArticle ".json_encode($this->MongoArticle));
 ...
 */
	public function testGroupBy() {
		...
		$query = array(
			...
			'reduce' => 'function(obj, result){result.csum += 1;}',
			'conditions' => array('count' => array('$lt' => $cond_count)),
		);
 ...
 */
	public function testMapReduce() {
		...
		$map = new MongoDB\BSON\Javascript("function() { emit(this.title,1); }");
		...
		$mongo = $this->Post->getDataSource();
		$reply = $mongo->mapReduce($params);
		$this->assertEqual($reply['result'], 'test_mapreduce_posts');
		$this->assertEqual($reply['ok'], 1);
		...
		$reply = $this->Post->query($params);
		$this->assertTrue(is_array($reply['results']));
		$this->assertEqual($reply['ok'], 1);

		$posts = array();
		foreach ($reply['results'] as $post) {
 ...
 */
	public function testSchemaless() {
		...
		$this->MongoArticle = ClassRegistry::init('MongoArticle');
		$this->MongoArticle->create();
		$this->assertNotEmpty($this->MongoArticle->save($toSave), 'Saving with no defined schema failed');

		$expected = array_intersect_key($toSave, array_flip(array('title', 'body', 'tags')));
		$result = $this->MongoArticle->read(array('title', 'body', 'tags'));
		syslog(LOG_NOTICE, 'Schemaless read '.json_encode(array_keys($result['MongoArticle'])));
 ...
 */
	public function testRegexSearch() {
		...
		$count = $this->MongoArticle->find('count', array(
			'conditions' => array(
				'title' => new MongoDB\BSON\Regex('^Article', '')
			)
		));
 ...
 */
	public function testEmptyReturn() {
		...
		// http://www.codeproject.com/Articles/987758/Changes-to-Model-find-first-in-CakePHP
		// find('first') with not matching conditions resutns array().
		$this->assertTrue(is_array($articles));
 ...
 */
	public function testSaveUniques() {
		...
		$this->Post->Behaviors->attach('MongoDBLib.SqlCompatible');
		$this->Post->create();
		$saveResult = $this->Post->save($saveData);
		$this->assertNotEmpty($saveResult);
